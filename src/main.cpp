...
uint256 hashGenesisBlock("0xffc13d6f2265c8dbf32d8edd4e89aa5aca8b93c2cb692d73c23e0fa82f46d80f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // GammaCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "GammaCoin Signed Message:\n";
...
{
    // GammaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    //nSubsidy >>= (nHeight / 840000); // GammaCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.00173611 * 24 * 60 * 60; // GammaCoin: 3.5 days
static const int64 nTargetSpacing = 120; // GammaCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // GammaCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // GammaCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // GammaCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe59e0c4fbbd2a15f7b4142edc1b8fc3b65e77844d80af2df84332171f23c08bd");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Flight MH370 Is Supposedly 'Found'";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1396433790;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 147834;

        if (fTestNet)
        {
            block.nTime    = 1396423348;
            block.nNonce   = 15406;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x0ffcab6dfe44ecff3bb4f656e1d889ed16d8586f8a484ea1b62efb32c46f197a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // GammaCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GammaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GammaCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GammaCoinMiner(CWallet *pwallet)
{
    printf("GammaCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gammacoin-miner");
    ...
    try { loop {
        ...
        printf("Running GammaCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GammaCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GammaCoinMiner, pwallet));
}
